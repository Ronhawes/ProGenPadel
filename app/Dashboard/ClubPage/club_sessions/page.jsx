...
export default function ResourcesPage() {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        const playerId = localStorage.getItem("clubId")
        if (!playerId) return alert("club ID not found in localStorage")

        const res = await fetch(`/api/Sessions/club_sessions?id=${playerId}`)
                ...
                item?.court_name ? (
                  ...
                  >
                    ...
                    <p className="mt-2 text-base sm:text-lg text-neutral-300">
                      ...
                      Court Charges: KES <strong>{item.court_charges}</strong><br />
                      Total Charges: KES <strong>{item.charges}</strong><br />
