...
const UpdatePlayerForm = () => {
  ...
  useEffect(() => {
    const fetchPlayer = async () => {
      ...
      try {
        const res = await fetch(`/api/Coaches/get?id=${coachId}`);
        const data = await res.json();

        if (!res.ok) throw new Error(data.error || 'Failed to fetch coach');
    ...
    <div className="min-h-screen flex items-center justify-center p-2">
      <div className="w-full max-w-md p-6 sm:p-8 rounded-2xl border border-white/30 shadow-2xl bg-white/10 backdrop-blur-lg text-black space-y-6">
        ...
        <div className="text-center mt-3">
          <a href="/Dashboard/CoachPage" className="text-sm text-teal-500 hover:text-teal-300 font-semibold">
            ← Back to Dashboard
          </a>
        </div>
      </div>
